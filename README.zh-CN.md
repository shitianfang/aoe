...
提示词与记忆，区别于它不可变的基础提示词——都不出这台机器。AOE 自己只往外发两样东西：模型调用，和一个网页字体；至于智能体自己的工具会去
取什么，那取决于你派给它的活。
...
npm install
cp .env.example .env   # 把里面的占位符换成真的 NVIDIA NIM key —— https://build.nvidia.com
...
是内置的；另外任何 OpenAI 兼容的端点也能用，但要手写进 `~/.prime/agent/models.json`。上面那个
NVIDIA NIM key 是纯模型兜底用的，不是运行时的供应商。
...
> bridge 只绑 `127.0.0.1`，但它不校验来源，所以你浏览器里开着的任何一个页面都能驱动你的智能体。
> `npm run dev` 还把渲染层开在 `0.0.0.0` 上、关掉了 host 校验，并把 `/bridge` 直通转发——任何能
> 碰到 3000 端口的东西，权限一样大。只在可信网络里用。
>
> 想让它停下来：智能体在干活、且输入框是空的时候，发送按钮就是**停止**，按下即中止本轮——你一开始
> 打字它又变回发送，那是插话引导、不是停。关掉应用停不了任何东西：worker 是常驻的。清掉目标、或把
> 自动运行关掉，它才不会再自己续上；`pkill -f prime-agent` 是全部结束，只剩 Python kernel。
...
- **自动学习**是一个开关，管整台机器：所有智能体、所有工作区。它写的就是运行时自己那份全局设置，
  bridge 随即重载每个活着的 root worker，让改动当场生效。面板交代节奏：上一次自动复盘发生在什么时候，
  下一次最早什么时候能发生。
...
两个操作。**回滚**一步撤销一条经验，这次回滚本身也会被记成一条经验，而这一条，它自己的详情面板不会再给
...
显式发布的东西：什么变了、谁改的、什么时候。扫描最深只走四层，且跳过所有点开头的条目以及 `node_modules`、`.git`，所以写到
...
这个客户端对「活该怎么干」并不中立。AOE 建的每个会话都会被追加一段系统提示词——是追加，永远
不替换运行时自己那份——它围着三件事写，因为少掉任何一件，翻车的样子都是固定的：一个没人要的
漂亮东西、一个没人看过的坏东西、或者五轮下来什么都没变。

**对齐。** 凡是有形状的东西，第一轮写四个**真正不同**的版本，各成一个文件、各自发布，一版一行
说它牺牲了什么，**推荐其中一个并说明理由**，然后停下来让人挑。这一轮只规划，不开做。用户一挑
完，智能体先写 `.review/brief.md`——选了哪版、放弃了什么、三条**可核对**的判据——这个文件不
存在就不许动工。后面每一轮、每一个评委、最后那份汇报，都拿它当尺子。

**验证。** 没有任何东西能靠「作者读自己写的源码」就发布。
`GET /bridge/shot?path=<文件>&out=<png>` 会用 Preview 同一套渲染把候选离屏画出来，回给你一张
PNG，外加这一页加载时出的岔子：图没加载出来、在目标宽度下横向滚动、控制台报错、渲染出来几乎
没有字。智能体用 `attach_image` **看**那张图，再对着判据一条一条回答，写进 `.review/check.md`；
有一条不过的候选，根本不会送到评委面前。机器上没有渲染器时，shot 会明说，那一轮记成「未验证」，
而不是悄悄跳过。

**迭代。** 一轮先把上一版从磁盘读出来，点名 3–5 个属性的 before→after（其中至少一个大到在缩略图
上能看见），然后产出**两到三个互斥方向的突变体**——「更透气」和「更密」只能有一个对。闸门会把
diff 里兑现不了自己目标的候选挡回去。然后子代理评委**看图**盲选：洗过牌、去掉来源，三种视角
（能不能干活 / 做得精不精 / 先坏在哪），回答 `WINNER`、`GLANCE`（两秒能看出差别吗）、`SAW`
（看的是图还是源码），以及对抗视角的 `BREAKS`——赢家仍然踩了哪条判据，一旦点名，票数再高也一票
否决。只有赢家才写回正式文件并发布。连输两轮不是再来第三个变体，而是回到对齐那轮的备选版本换
赛道；等判据全过、且一轮没人能击败现任版本，那就是做完了，而且要说出来。这一路学到的东西交给
`refine.run(…)` 留下来，跨任务生效——它会出现在自进化那一列，你可以读它，也可以回滚它。

展开的长版本是本仓库里的一个技能：[`skills/aoe-way`](skills/aoe-way/SKILL.md)，bridge 会把它和
运行时自带的技能一起交给守护进程。上面每一件事在里面都有可直接运行的写法：闸门、截图、盲评面板、
计票、什么时候停。改它，你工作区里的智能体干活方式就跟着变。追加的提示词在会话创建时固定下来，
所以改动对新会话立刻生效，对已经在跑的 master 则要等它下次冷启动；技能文件同理。
...
- **中间最多四屏，2×2 网格，每屏一组 tab。** 把 tab——或者直接把左列的一个智能体——拖过某一屏的
  边缘就在那里分屏，拖到中间则是加成一个 tab。对话类的每一屏有自己的输入框，绑定它显示的
  那个智能体，整套布局按工作区保存。
...
只取三样东西——`packages/coding-agent/dist/index.js` 当 SDK、没有守护进程时用
`packages/coding-agent/dist/cli.js` 把它拉起来、`packages/coding-agent/skills/` 作为技能目录的来源
之一——所以 `PRIME_AGENT_DIR` 指向任何一份构建好的 prime-agent 都能跑。
...
| 连接状态里的 `autonomous` | 自动运行面板里的实时读数 | 回来是 `null`；面板照样给出上限和开启按钮，但不再报告进度 |
